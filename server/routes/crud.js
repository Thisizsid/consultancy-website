 ...
 */
router.get('/:collection/:id', validateCollection, (req, res) => {
...
router.post('/:collection', validateCollection, conditionalAuth, (req, res) => {
  const col = req.params.collection;
  const data = req.body;
  const db = getDb();
 ...
 */
router.put('/:collection/:id', validateCollection, verifyTokenMiddleware, (req, res) => {
  const { collection: col, id } = req.params;
